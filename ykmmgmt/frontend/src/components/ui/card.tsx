...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-[var(--card-spacing)] overflow-hidden rounded-xl bg-card py-[var(--card-spacing)] text-sm text-card-foreground ring-1 ring-foreground/10 [--card-spacing:1rem] data-[size=sm]:[--card-spacing:0.75rem] [&:has([data-slot=card-footer])]:pb-0 [&>img:first-child]:rounded-t-xl [&>img:last-child]:rounded-b-xl [&>img:first-child]:pt-0",
        className,
      )}
      {...props}
    />
